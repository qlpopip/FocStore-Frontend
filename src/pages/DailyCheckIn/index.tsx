...
const DailyCheckIn: React.FC = () => {
    ...
    try {
      if (account) {
        ...
        if (data[0] && attendance[0]) {
          ...
          (data[1] && data[1].status_code === 401) ||
          (attendance[1] && attendance[1].status_code === 401)
        ) {
  ...
  useEffect(() => {
    const calculatePoints = () => {
      ...
      let lastPoint = 0;
      const sortedDailyCheck = attendanceList.sort(
        (a, b) =>
      ...
      sortedDailyCheck.forEach((checkIn) => {
        ...
        if (
          !sortedDailyCheck.find(
            (d) =>
              new Date(d.created_at).toISOString().split("T")[0] === nextDayStr
          )
        ) {
  ...
  const point =
    streakPoints[yesterday] &&
    streakPoints[dayBeforeYesterday] &&
    streakPoints[twoDaysBeforeYesterday] &&
    streakPoints[threeDaysBeforeYesterday]
      ? 40
      : streakPoints[yesterday] &&
        streakPoints[dayBeforeYesterday] &&
        streakPoints[twoDaysBeforeYesterday]
      ? 30
      : streakPoints[yesterday] && streakPoints[dayBeforeYesterday]
      ? 20
      : streakPoints[yesterday]
      ? 10
      : 0;
  ...
  for (let i = 0; i < firstDayOfMonth; i++) {
    const dateStr = `${currentYear}-${String(currentMonth).padStart(
      2,
      "0"
    )}-${String(prevMonthDay + i).padStart(2, "0")}`;
    const dateStrRender = `${String(currentMonth).padStart(2, "0")}-${String(
      prevMonthDay + i
    ).padStart(2, "0")}`;
    const points = streakPoints[dateStr] || null;
    calendarDays.push(
      <div
        key={`prev-month-day-${prevMonthDay + i}`}
        className={`calendar-day prev-month ${!points && "missed"}`}
      >
  ...
  for (let day = 1; day <= daysInMonth; day++) {
    ...
    const dateStrRender = `${String(currentMonth + 1).padStart(
      2,
      "0"
    )}-${String(day).padStart(2, "0")}`;
    ...
    calendarDays.push(
      <div
        ...
        className={`calendar-day ${isToday ? "today" : ""} ${
          !streakPoints[dateStr] && isPastOrToday && "missed"
        }`}
      >
        <p className="streak_calendar_day">{dateStrRender}</p>
        ...
        <div className="daily_point">
          {isPastOrToday ? (
            streakPoints[dateStr] ? (
              <p className={`point collected`}>
                {"+" + streakPoints[dateStr]}{" "}
              </p>
            ) : (
              <Image src={IconsFile.Missed} alt="" className="point_missed" />
          ...
          <p
            className={`text ${
              isPastOrToday && streakPoints[dateStr] && "collected"
            }`}
          >
            {" "}
            {isPastOrToday
              ? streakPoints[dateStr]
                ? "Collected"
                : "Missed"
              : streakPoints[todayStr] && isToday
              ? "Collected"
              : ""}
          </p>
          {isPastOrToday ? (
            streakPoints[dateStr] ? (
              <Button className="more">Collected </Button>
            ) : (
              <Button className="more">Missed </Button>
            )
          ) : (
            <Button
              className="more"
              onClick={() =>
                !streakPoints[todayStr] && isToday && onClickCollect()
              }
            >
              {streakPoints[todayStr] && isToday ? "Collected" : "Collect"}{" "}
            </Button>
          )}
        </div>
      </div>
  ...
  for (let i = 1; i <= daysFromNextMonthToAdd; i++) {
    ...
    const dateStr = `${currentYear}-${String(currentMonth + 2).padStart(
      2,
      "0"
    )}-${String(nextMonthDay).padStart(2, "0")}`;
    const dateStrRender = `${String(currentMonth + 2).padStart(
      2,
      "0"
    )}-${String(nextMonthDay).padStart(2, "0")}`;
    ...
    calendarDays.push(
      <div
        ...
        className="calendar-day next-month empty"
      >
  ...
  return (
    <div>
      <Layout id="daily-check-in">
        <Navigator navigation="Daily Check-In" />
              ...
              <div className="points_box">
                ...
                <div className="streak_calendar">
                  <div className="top">
                    ...
                    <div
                      className="check_in_calendar"
                      onClick={() => setShowCalendar(true)}
                    >
                      <Image
                        src={IconsFile.Calendar}
                        alt=""
                  ...
                  <div className="card_box">
                    {calendarDays.filter(
                      (day) => !day.props.className.includes("empty")
                    )}
                  </div>
                </div>
              </div>
