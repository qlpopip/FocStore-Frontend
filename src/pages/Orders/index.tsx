        ...
        (orders.length > 0 ?
          <SaidBar>
            <div className="order_container ">
              {orders.map(order => (
                <div className={`checkout_box ${showOrder.index === order.id && showOrder.status && "open"}`}
                  style={{ border: `2px solid ${colorsBorder(order.status)}` }} key={order.id}
                >
                  <div className="title_box" onClick={() => onClickOpenOrder(order.id)}>
                    <p className="title" > Order Information status - {order.status} </p>
                    ...
                    <div className="order_box">
                      ...
                      <div className="orders">
                        {order.orderProducts.map((item) => (
                          <div className="order" key={item.id} onClick={() => navigate(`/product/${item.product.id}`)}>
